...
<div class='table-responsive'>
    <table id='table-detail' class='table table-striped'>
        ...
        $file_location = base_path('mixtra/views/default/type_components/'.$type.'/component_detail.blade.php');
//        $user_location = resource_path('mixtra/type_components/'.$type.'/component_detail.blade.php');
        ?>

        @if(file_exists($file_location))
            <?php $containTR = (substr(trim(file_get_contents($file_location)), 0, 4) == '<tr>') ? TRUE : FALSE;?>
            @if($containTR)
                @include('mitbooster::default.type_components.'.$type.'.component_detail')
            @else
                <tr>
                    <td>{{$form['label']}}</td>
                    <td>@include('mitbooster::default.type_components.'.$type.'.component_detail')</td>
                </tr>
            @endif
