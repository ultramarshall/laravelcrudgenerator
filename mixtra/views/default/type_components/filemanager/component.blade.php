...
@endif
    ...
    <div class="{{$col_width?:'col-sm-10'}}">
        @if($value)
            ...
            ?>
            <p><a data-lightbox='roadtrip' href='{{ $url}}'><object style='max-width:160px' title="File For {{$form['label']}}" src='{{$url}}'/></a></p>
            <?php else:?>
            ...
            @if(!$readonly || !$disabled)
                <p><a class='btn btn-danger btn-delete btn-sm' onclick="if(!confirm('{{trans("mixtra.delete_title_confirm")}}')) return false"
                      href='{{url(MITBooster::mainpath("delete-file?file=".$value."&id=".$row->id."&column=".$name))}}'><i
