...
@endif
    ...
    <div class="{{$col_width?:'col-sm-10'}}">
        ...
        @if(!$form['dataenum'] && !$form['datatable'] && !$form['dataquery'])
            <em>{{trans('mixtra.there_is_no_option')}}</em>
        @endif
            ...
            @foreach($dataenum as $k=>$d)
                ...
                <div class=" {{$disabled}}">
                    <label class='radio-inline'>
                        <input type="radio" class="m-r-10" {{$disabled}} {{$checked}} name="{{$name}}" value="{{$val}}"> {{$label}}
                    </label>
                </div>
            @endforeach
        @endif
        ...
        if (@$form['datatable']):
            ...
            if (MITBooster::isColumnExists($tables[0], 'deleted_at')) {
            ...
            if (count($tables)) {
                for ($i = 1; $i <= count($tables) - 1; $i++) {
                    ...
                    $fk_field = MITBooster::getForeignKey($parent_table, $tab);
                    $pk = MITBooster::findPrimaryKey($tab) ?: 'id';
