...
@section('content')
    ...
    <ul class="nav nav-tabs"  role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link active" href="{{ MITBooster::mainpath('screet-key') }}" role="tab"><span class="hidden-sm-up"><i class="fa fa-key"></i></span> <span class="hidden-xs-down">API Secret Key</span></a> 
        </li>
        <li class="nav-item">
            <a class="nav-link" href="{{ MITBooster::mainpath('generator') }}" role="tab"><span class="hidden-sm-up"><i class="fa fa-cog"></i></span> <span class="hidden-xs-down">API Generator</span></a> 
        </li>
    </ul>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($row->status == 'active')
                                <a class='btn btn-xs btn-default' href='{{ MITBooster::mainpath("status-apikey?id=$row->id&status=0") }}'>Non Active</a>
                            @else
                                <a class='btn btn-xs btn-default' href='{{ MITBooster::mainpath("status-apikey?id=$row->id&status=1") }}'>Active</a>
                            @endif
                ...
                <script>
                    ...
                    function generate_screet_key() {
                        $.get("<?php echo route('ApiCustomControllerGetGenerateScreetKey')?>", function (resp) {
                            ...
                            $('#table-apikey').append("<tr><td>" + lastno + "</td><td>" + resp.key + "</td><td>0</td><td><span class='label label-success'>Active</span></td><td>" +
                                "<a class='btn btn-xs btn-default' href='{{MITBooster::mainpath("status-apikey")}}?id=" + resp.id + "&status=0'>Non Active</a> <a class='btn btn-xs btn-danger' href='javascript:void(0)' onclick='deleteApi(" + resp.id + ")'>Delete</a> </td></tr>"
                            );
                            $('.no-screetkey').remove();
                    ...
                    function deleteApi(id) {
                        swal({
                            ...
                            $.get("{{MITBooster::mainpath('delete-api-key')}}?id=" + id, function (resp) {
                                if (resp.status == 1) {
