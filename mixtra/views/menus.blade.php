...
@section('content')
    ...
    @push('bottom')
        ...
        <script type="text/javascript">
            $(function () {
                var mit_privileges_id = '{{$mit_privileges_id}}';
                var sortactive = $(".draggable-menu").sortable({
                    ...
                    onDrop: function ($item, container, _super) {
                        ...
                        $.get("{{route('MenusControllerGetSaveMenu')}}", {menus: jsonString, isActive: isActive}, function (resp) {
    ...
    <div class='row'>
                ...
                <div class="card-body clearfix">
                    <ul class='draggable-menu draggable-menu-active'>
                        @foreach($menu_active as $menu)
                            @php
                                $privileges = DB::table('mit_menus_privileges')
                                ->join('mit_privileges','mit_privileges.id','=','mit_menus_privileges.mit_privileges_id')
                                ->where('mit_menus_id',$menu->id)->pluck('mit_privileges.name')->toArray();
                            @endphp
                            <li data-id='{{$menu->id}}' data-name='{{$menu->name}}'>
                                <div class='{{$menu->is_dashboard?"is-dashboard":""}}' title="{{$menu->is_dashboard?'This is setted as Dashboard':''}}">
                                    <i class='{{($menu->is_dashboard)?"icon-is-dashboard fa fa-dashboard":$menu->icon}}'></i> {{$menu->name}} <span
                                            class='float-right'><a class='fa fa-pencil-alt' title='Edit'
                                                                  href='{{route("MenusControllerGetEdit",["id"=>$menu->id])}}?return_url={{urlencode(Request::fullUrl())}}'></a>&nbsp;&nbsp;<a
                                                title='Delete' class='fa fa-trash'
                                                onclick='{{MITBooster::deleteConfirm(route("MenusControllerGetDelete",["id"=>$menu->id]))}}'
                                    ...
                                    @if($menu->children)
                                        @foreach($menu->children as $child)
                                            @php
                                                $privileges = DB::table('mit_menus_privileges')
                                                ->join('mit_privileges','mit_privileges.id','=','mit_menus_privileges.mit_privileges_id')
                                                ->where('mit_menus_id',$child->id)->pluck('mit_privileges.name')->toArray();
                                            @endphp
                                            <li data-id='{{$child->id}}' data-name='{{$child->name}}'>
                                                <div class='{{$child->is_dashboard?"is-dashboard":""}}'
                                                    ...
                                                    <span class='float-right'><a class='fa fa-pencil-alt' title='Edit'
                                                                                href='{{route("MenusControllerGetEdit",["id"=>$child->id])}}?return_url={{urlencode(Request::fullUrl())}}'></a>&nbsp;&nbsp;<a
                                                                title="Delete" class='fa fa-trash'
                                                                onclick='{{MITBooster::deleteConfirm(route("MenusControllerGetDelete",["id"=>$child->id]))}}'
                                                                href='javascript:void(0)'></a></span>
                                                    <br/><em class="text-muted">
                                                        <small><i class="fa fa-users"></i> &nbsp; {{implode(', ',$privileges)}}</small>
                                                    </em>
                            ...
                            <li data-id='{{$menu->id}}' data-name='{{$menu->name}}'>
                                <div><i class='{{$menu->icon}}'></i> {{$menu->name}} 
                                    <span class='float-right'>
                                        <a class='fa fa-pencil-alt' title='Edit' href='{{route("MenusControllerGetEdit",["id"=>$menu->id])}}?return_url={{urlencode(Request::fullUrl())}}'></a>&nbsp;&nbsp;
                                        <a title='Delete' class='fa fa-trash' onclick='{{MITBooster::deleteConfirm(route("MenusControllerGetDelete",["id"=>$menu->id]))}}' href='javascript:void(0)'></a>
                                    </span>
                                </div>
                                <ul>
                                    @if($menu->children)
                                        @foreach($menu->children as $child)
                                            <li data-id='{{$child->id}}' data-name='{{$child->name}}'>
                                                <div><i class='{{$child->icon}}'></i> {{$child->name}} 
                                                    <span class='float-right'>
                                                        <a class='fa fa-pencil-alt' title='Edit' href='{{route("MenusControllerGetEdit",["id"=>$child->id])}}?return_url={{urlencode(Request::fullUrl())}}'></a>&nbsp;&nbsp;
                                                        <a title="Delete" class='fa fa-trash' onclick='{{MITBooster::deleteConfirm(route("MenusControllerGetDelete",["id"=>$child->id]))}}' href='javascript:void(0)'></a>
                                                    </span>
                                                </div>
                                            </li>
                                        @endforeach
        ...
        <div class="col-sm-7">
            <div class="card panel-primary">
                ...
                <div class="card-body">
                    <form class='form-horizontal' method='post' id="form" enctype="multipart/form-data" action='{{MITBooster::mainpath("add-save")}}'>
