...
class ApiController extends Controller
{
    ...
    public function execute_api($output = 'JSON')
    {
        ...
        $row_api = DB::table('mit_apicustom')->where('permalink', $this->permalink)->first();

        $action_type = $row_api->action;
        $table = $row_api->table_name;
        $pk = MITBooster::pk($table);
        ...
        */
        if ($parameters) {
            ...
            foreach ($parameters as $param) {
                ...
                if ($type == 'exists') {
                    ...
                    $table_exist = MITBooster::parseSqlTable($table_exist)['table'];
                ...
                if ($name == 'id') {
                    $table_exist = MITBooster::parseSqlTable($table)['table'];
                    $table_exist_pk = MITBooster::pk($table_exist);
        ...
        $uploads_format_candidate = explode(',', config("mixtra.UPLOAD_TYPES"));
        $uploads_candidate = explode(',', config('mixtra.IMAGE_FIELDS_CANDIDATE'));
        ...
        if ($action_type == 'list' || $action_type == 'detail' || $action_type == 'delete') {
            ...
            foreach ($responses as $resp) {
                ...
                if ($used == 0 && ! MITBooster::isForeignKey($name)) {
                    continue;
                }
                ...
                $name_tmp[] = $name;
                if (MITBooster::isForeignKey($name)) {
                    $jointable = MITBooster::getTableForeignKey($name);
                    $jointable_field = MITBooster::getTableColumns($jointable);
                    $jointablePK = MITBooster::pk($jointable);
            ...
            if (MITBooster::isColumnExists($table, 'deleted_at')) {
            ...
            $data->where(function ($w) use ($parameters, $posts, $table, $type_except) {
                foreach ($parameters as $param) {
                    ...
                    if ($required == '1') {
                        if (MITBooster::isColumnExists($table, $name)) {
            ...
            if ($row_api->condition) {
                $theSql = $row_api->condition;
            ...
            if ($action_type == 'list') {
                ...
                if ($rows) {
                    ...
                    if (MITBooster::getSetting('api_debug_mode') == 'true') {
            ...
            foreach ($input_validator as $k => $v) {
                if (MITBooster::isColumnExists($table, $k)) {
                    $row_assign[$k] = $v;
                }
            }
            ...
            if ($action_type == 'save_add') {
                if (MITBooster::isColumnExists($table, 'created_at')) {
                    $row_assign['created_at'] = date('Y-m-d H:i:s');
                }
            }

            if ($action_type == 'save_edit') {
                if (MITBooster::isColumnExists($table, 'updated_at')) {
            ...
            foreach ($parameters as $param) {
                ...
                if ($type == 'file' || $type == 'image') {
                    $row_assign[$name] = MITBooster::uploadFile($name, true);
                } elseif ($type == 'base64_file') {
                    $row_assign[$name] = MITBooster::uploadBase64($value);
            ...
            if ($action_type == 'save_add') {
                ...
                try {
                    ...
                    if ($row_api->condition) {
                        $update->whereraw($row_api->condition);
                    }
            ...
            foreach ($parameters as $param) {
                ...
                $type = $param['type'];
                if ($type == 'ref') {
                    if (MITBooster::isColumnExists($config, 'id_'.$table)) {
                        DB::table($config)->where($name, $value)->update(['id_'.$table => $lastId]);
                    } elseif (MITBooster::isColumnExists($config, $table.'_id')) {
