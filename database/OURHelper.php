...
class OURHelper
{
	public static function setPerfomance($id)
	{
		...
		if($detail3 != null) {
			...
			if($detail3->diff != null)
			if($detail3->diff != 0)
				$co = $detail3->diff/$changeOver*100;
			//dd($detail3->diff);
			//dd(411/720);
		}
    ...
    public static function latestSchedule($id, $our_items_id, $is_same, $our_lines_id, $work_hour, $start_date, $prefered_date)
    {
		...
		$latest['end_date'] = self::endScheduler($id, $latest['start_date']
			, (int)($work_hour * 3600)
			, $our_shifts_id
			, $our_lines_id);
    ...
    public static function startSchedule($date, $our_shifts_id, $our_lines_id, $changeOver) {
		...
		//dd(date('Y-m-d H:i:s', strtotime($end_date)));
		...
		} else if(date('Y-m-d H:i:s', strtotime($date)) > date('Y-m-d H:i:s', strtotime($end_date))) {
			// dd('bbb');
    ...
    public static function startOvertime($date, $our_shifts_id, $our_lines_id, $changeOver) {
		$overtime = DB::table('our_overtime')
			...
			->first();
		if($overtime == null)
			return self::startSchedule(date('Y-m-d', strtotime('+1 day', strtotime($date)))
				, $our_shifts_id
				, $our_lines_id
				, $changeOver);	
		...
		if(date('Y-m-d H:i:s', strtotime($date)) < date('Y-m-d H:i:s', strtotime($overtime->start_hour))) {
			$date = date('Y-m-d', strtotime($date));
		...
		} else if(date('H:i:s', strtotime($date)) < date('H:i:s', strtotime($overtime->end_hour))) {
			return $date;
		} else {
			return self::startSchedule(date('Y-m-d', strtotime('+1 day', strtotime($date)))
				, $our_shifts_id
    ...
    public static function endScheduler($id, $date, $seconds, $our_shifts_id, $our_lines_id) {
		...
		if($working_time == null) {
    		if(!self::endOvertime($date, $start_date, $end_date, $seconds, $our_lines_id))  {
				...
				return self::endScheduler($id, date('Y-m-d H:i:s', strtotime('+1 day', strtotime($result_date)))
					, $seconds
					, $our_shifts_id
					, $our_lines_id);
			}
			...
			return self::endScheduler($id, $end_date
				, $seconds
				, $our_shifts_id
				, $our_lines_id);
		}
		...
		if($holiday != null) {
    		if(!self::endOvertime($date, $start_date, $end_date, $seconds, $our_lines_id))
				return self::endScheduler($id, date('Y-m-d', strtotime('+1 day', strtotime($date)))
					, $seconds
					, $our_shifts_id
					, $our_lines_id);
		}
    	...
    	if(!self::endOvertime($date, $start_date, $end_date, $seconds, $our_lines_id)){
			...
			// Log::info($initial_date);
			// Log::info($result_date);
			return self::endScheduler($id, $result_date
				, $seconds
				, $our_shifts_id
				, $our_lines_id);
    	}
    ...
    public static function endOvertime(&$date, &$start_date, &$end_date, &$seconds, $our_lines_id) {
		...
		if(date('H:i:s', strtotime($date)) <= date('H:i:s', strtotime($start_date))) {
			...
			$start_date = date('Y-m-d', strtotime($date));
			$start_date = date('Y-m-d H:i:s', strtotime('+'.$hours.' hour +'.$minutes.' minutes', strtotime($start_date)));
			// Log::info('Masuk overtime');
			return true;
		} else if(date('H:i:s', strtotime($date)) > date('H:i:s', strtotime($start_date))) {
			return false;	
		}
    }
    ...
    public static function savePlans($id, $start_date, $end_date, $working_time) {
		...
		$hours = 0;
		$minutes = 0;
		if(date('Y-m-d H:i:s', strtotime($start_date)) == date('Y-m-d 00:00:00', strtotime($start_date))) {
			$hours = substr($working_time->start_hour, 0, 2); 
			$minutes = substr($working_time->start_hour, 3, 2); 
		...
		// Log::info('-----');
		if(date('Y-m-d', strtotime($order->start_date)) == date('Y-m-d', strtotime($order->end_date)))
			$diff = $order->work_hour;
		// Log::info($diff);
